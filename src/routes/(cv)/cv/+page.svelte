...
<Wrap>
	{#each $profile.getDegrees() as degree }
		<Item start={degree.start} stop={degree.end} header={degree.institution} two={degree.thesis} three={degree.committee}/>
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getJobs(job => job.academic, job => -job.startdate) as job }
		<Item start={job.startdate} stop={job.enddate} header={job.title} two={job.department} three={job.organization}/>
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getPublications(pub => pub.award !== undefined && pub.award.filter(award => award.includes("most influential paper")).length > 0, pub => -pub.year) as pub }
		<Item start={pub.year} header={pub.title} two={pub.award?.join(", ")} three={$profile.getSourceName(pub.source)}/>
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getFunding(funding => !funding.private, funding => funding.commitment.end === null ? -Infinity : -parseDate(funding.commitment.end).getFullYear()) as funding }
		{#if funding.commitment.start }
			<Item start={parseDate(funding.commitment.start).getFullYear()} stop={funding.commitment.end === null ? null : parseDate(funding.commitment.end).getFullYear()} header={funding.title} two={funding.amount} three={funding.funder} four={funding.award ?? ""} five={funding.investigators} six={funding.description} />
		{/if}
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getImpacts(impact => impact.kind === "press", impact => -impact.start) as impact }
		<Item start={impact.start} stop={impact.end} header={impact.title ?? ""} two={impact.author} three={impact.source}/>
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getTalks().filter(talk => talk.keynote) as keynote }
		<Item start={parseDate(keynote.date).getFullYear()} header={keynote.title} two={keynote.venue}/>
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getPanels() as panel }
		<Item start={parseDate(panel.date).getFullYear()} header={panel.title} two={panel.venue}/>
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getPatents() as patent }
		<Item start={parseInt(patent.year)} header={patent.title} two={patent.number} three={patent.inventors} />
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getClasses() as course }
		<Item start={course.offerings.sort((a, b) => a.year - b.year)[0].year} stop={course.offerings.sort((a, b) => b.year - a.year)[0].year} header={`${course.number} ${course.title}`} two={course.level} three={course.description} four={"Taught " + course.offerings.filter(offer => offer.score !== null).length + " times"}/>
	{/each}
</Wrap>
...
<Wrap>
	{#each $profile.getPeople(
		...
		<Item start={person.startdate} stop={person.enddate} header={person.name} two={person.dept} three={person.coadvisor ?? undefined} four={person.achievements}/>
	{/each}
</Wrap>
...
{#each $profile.getEditing(role => role.type === "journal", role => role.commitment.start === null ? -Infinity : -(parseDate(role.commitment.start).getFullYear())) as board }
	{#if board.commitment.start }
		<Item start={parseDate(board.commitment.start).getFullYear()} stop={board.commitment.end === null ? null : parseDate(board.commitment.end).getFullYear()} header={board.venue} two={board.title} />
	{/if}
{/each}
...
{#each $profile.getEditing(role => role.type === "conference", role => role.commitment.start === null ? -Infinity : -(parseDate(role.commitment.start).getFullYear())) as board }
	{#if board.commitment.start }
		<Item start={parseDate(board.commitment.start).getFullYear()} stop={board.commitment.end === null ? null : parseDate(board.commitment.end).getFullYear()} header={$profile.getSourceName(board.venue)} two={board.title} />
	{/if}
{/each}
...
<Wrap>
	{#each $profile.getReviewing(role => role.level === "pc", role => -role.years.sort().reverse()[0]) as role }
		<Item start={role.years.sort()[0]} stop={role.years.sort().reverse()[0]} header={$profile.getSourceName(role.venue)} two={role.title} />
	{/each}
</Wrap>
...
{#each $profile.getService(service => service.level === "international", service => service.commitment.end ? -(parseDate(service.commitment.end).getTime()) : Number.NEGATIVE_INFINITY) as service }
	{#if service.commitment.start }
		<Item start={parseDate(service.commitment.start).getFullYear()} stop={service.commitment.end === null ? null : parseDate(service.commitment.end).getFullYear()} header={service.title} two={service.committee} three={service.description} />
	{/if}
{/each}
</Wrap>
