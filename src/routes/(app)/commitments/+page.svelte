    ...
    <tbody>
        ...
        {#each weeks as week, index }
            {#if index < 1 || weeks[index].date.getMonth() !== weeks[index - 1].date.getMonth()}
                <tr>
                    <td colSpan="2"><h4>{months[week.date.getMonth() + 1]} {week.date.getFullYear()}</h4></td>
                </tr>
            {/if}
            {@const hours = week.intersects.reduce((sum, intersect) => sum + (intersect.commitment.category === "personal" ? 0 : Math.round(intersect.commitment.hours * intersect.overlap)), 0) }
            <tr>
                <td>
                    <div class={hours > 45 ? "shake" : ""} style={`animationDelay: ${"-" + Math.round(10 * Math.random()) / 10 + "s"}`}>
                        {#each week.intersects as intersect}
                            <Bar commit={intersect.commitment} proportion={intersect.overlap} label={false}/>
                        {/each}
                    </div>
                    <div class="commitment-notes">
                        {#each week.intersects as intersect, index}
                            <span class={intersect.commitment.category}>
                                ...
                                {#if index < week.intersects.length - 1}<span> &sdot; </span>{/if}
                           </span>
                        {/each}
                    </div>
                </td>
                <td><div class={hours > 45 ? "shake" : ""}  style={`animationDelay: ${"-" + Math.round(10 * Math.random()) / 10 + "s"}`}>{hours + " hrs"}</div></td>
            </tr>
